...
class ClickHouseAnalyticsStore:
    ...
    def insert_trace(self, graph: Any) -> None:
        ...
        for span in graph.spans:
            ...
            span_rows.append([
                graph.tenant_id, graph.trace_id, span.span_id, span.parent_span_id,
                span.name, span.start_time, span.end_time, duration_ms,
                "error" if has_error else "success",
                str(span.error or attrs.get("error", "")) if has_error else None,
                fingerprint, input_keys, output_type,
                json.dumps({k: str(v) if not isinstance(v, (str, int, float, bool, type(None))) else v
        ...
        if span_rows:
            client.insert(
                ...
                span_rows,
                column_names=[
                    "tenant_id", "trace_id", "span_id", "parent_span_id", "name",
                    "start_time", "end_time", "duration_ms", "status", "error",
                    "fingerprint", "input_keys", "output_type", "attributes",
                ],
            )
        ...
        client.insert(
            "temporallayr_traces",
            [[graph.tenant_id, graph.trace_id, start_t, end_t,
              len(graph.spans), error_count, fingerprint]],
            column_names=["tenant_id", "trace_id", "start_time", "end_time",
                          "span_count", "error_count", "fingerprint"],
        )
    ...
    def get_latency_percentiles(self, tenant_id: str, hours: int = 24) -> list[dict[str, Any]]:
        ...
        cols = ["name", "call_count", "p50_ms", "p95_ms", "p99_ms", "avg_ms",
    ...
    def get_span_timeline(self, trace_id: str, tenant_id: str) -> list[dict[str, Any]]:
        ...
        cols = ["span_id", "parent_span_id", "name", "start_time", "end_time",
                "duration_ms", "status", "error", "attributes"]
        rows = []
        for row in result.result_rows:
            d = dict(zip(cols, row))
...
def configure_clickhouse(
    host: str, port: int = 8443, database: str = "default",
    username: str = "default", password: str = "", secure: bool = True,
